...
type PriceColProps = {
    ...
    onCheckout?: (packageName: string, packageDisplayName: string, price: number, existingSubscriptionId?: string) => void;
}
...
export const PriceCol = ({
    ...
    // Trial users without a payment method need Checkout (to collect card + VAT)
    const needsCheckout = !hasPaymentMethod && isTrialing && !isCurrentPlan;
    // Users with a payment method can use the direct update flow
    const canDirectUpdate = stripeSubscriptionId && newPriceId && !isCurrentPlan && !needsCheckout;
    ...
    return (
        ...
        >
            ...
            {/* Trial user without payment method → inline checkout modal (collects card + VAT) */}
            {needsCheckout && userId && organizationId && email ? (
                <button
                    onClick={() => onCheckout?.(config.id, config.displayName, config.pricing?.monthly || 0, stripeSubscriptionId || undefined)}
                    className="mt-10 w-full font-semibold py-2 px-4 rounded-lg transition-colors bg-cyan-600 hover:bg-cyan-700 text-white"
                >
