...
export function SitemapTree({ tree, showRoot = true }: SitemapTreeProps) {
  ...
  const filterTree = useCallback(
    (node: SitemapNode, q: string): SitemapNode | null => {
      if (!q) return node;
      const ql = q.toLowerCase();
      const pagesMatch = (node.pages ?? []).some(
        (p) =>
      ...
      if (nodeMatch || pagesMatch || children.length > 0) {
        const filteredPages = (node.pages ?? []).filter(
          (p) =>
            (p.title ?? "").toLowerCase().includes(ql) ||
            (p.url ?? "").toLowerCase().includes(ql)
        );
        return { ...node, pages: filteredPages, children };
      }
      return null;
    },
    []
    ...
    <div className="border border-slate-200 rounded-lg p-3 bg-white">
      ...
      <div role="tree" aria-label="Sitemap tree" className="text-sm">
        {showRoot ? (
          ...
          (filtered ? filtered.children ?? [] : tree.children ?? []).map(
            (ch) => (
              <TreeNode
                key={ch.path}
                node={ch}
