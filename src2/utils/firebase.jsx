...
// If running in Vite dev mode and a Firestore emulator is configured, connect to it
if (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.DEV) {
  const fsEmRaw = import.meta.env.VITE_FIRESTORE_EMULATOR_HOST || ''
  if (fsEmRaw) {
...
// If running in Vite dev mode and a functions emulator is configured, connect to it
if (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.DEV) {
  const emHostRaw = import.meta.env.VITE_FUNCTIONS_EMULATOR_HOST || import.meta.env.VITE_FUNCTIONS_BASE_URL || ''
  if (emHostRaw) {
    const em = emHostRaw.replace(/^https?:\/\//, '')
    ...
    const unsub = onAuthStateChanged(auth, async (u)=>{
      if(u){
        ...
        try{
          ...
          console.log('[firebase] Firebase user ref', ref)
          const snap = await getDoc(ref)
          console.log('[firebase] Firebase snap', snap)
          const profile = snap.exists() ? snap.data() : {}
          console.log('[firebase] Firebase profile', profile)
