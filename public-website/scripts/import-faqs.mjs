 ...
 * 2. Run: node scripts/import-faqs.mjs
 * 
 * Or from the root:
 * cd public-website && node scripts/import-faqs.mjs
 */

import { createClient } from '@sanity/client'

// Get config from environment or use defaults
const projectId = process.env.NEXT_PUBLIC_SANITY_PROJECT_ID || '8x3jn6k7'
...
if (!token) {
  ...
  console.log('5. Run: SANITY_API_TOKEN="your-token" node scripts/import-faqs.mjs\n')
  process.exit(1)
}
...
const importFAQs = async () => {
  ...
  try {
    ...
    for (const faq of FAQ_DATA) {
      ...
      console.log(`  ✓ Created FAQ #${faq.order}: ${faq.question.substring(0, 60)}...`)
    }
